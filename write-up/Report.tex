...
\noindent
Improving 3D data acquisition, reconstruction and rendering methods is an ongoing, long-standing and highly demanded problem in computer vision. 
Increasing demand for efficient 3D reconstruction in applications such as augmented/virtual reality, autonomous devices 
(for purposes of navigation or perception in examples such as robotics or self-driving cars) or digital artistry (geometry acquisition for models in VFX, games or other digital products) 
has skyrocketed demand for research and improvements in this area.
...
Efficiency and accuracy improvements from advancements in computer hardware and machine-learning methods have made 3D reconstruction applicable
for a wider variety of tasks and hardware platforms; continuing this trend, one area of focus is made on
further reducing the number of input images required for reconstruction, thereby improving computational efficiency and reducing data acquisition hardware requirements,
this had lead to interest in the issue of novel view synthesis.

\noindent
Namely, in 2020 Neural Radiance Fields\cite{nerf} (NeRF's) became a method for low-input 3D reconstruction, capable of good quality 
novel view synthesis, by predicting a density and radiance volume, which can then be ray-traced to render novel views.
In 2023, 3D Gaussian Splatting\cite{3dgs} (3DGS) became an alternative method, offering major improvements in computational performance, instead predicting
a point cloud of gaussians containing parameters such as mean position, colour, rotation or size, where the gaussians are then rasterized to render novel views.
Developments in 3D Gaussian Splatting methods have allowed for 3D scene reconstruction using single or few RGB images.
While faster than other scene reconstruction techniques and requiring only a "one-shot" pass, 
these approaches often suffer from challenges such as layout/scale drift, over-smooth geometry and hallucinations in occluded regions. TODO: CITE
\newline
...
particularly in reconstructing structures unseen in the input view, including for views significantly different from the source. TODO: CITE
This project aims to address this by first researching inferable explicit geometry priors (such as planes, normals, visibility cues, depth, segmentation or edge maps) 
which can be dynamically produced for input images by existing specialized models, then proposing a lightweight augmentation for Splatter Image, with minimal architectural changes, allowing
predicted priors to be fed alongside the RGB images, allowing the priors to guide reconstruction in a more accurate manner,
compared to when the network has to learn these geometric features itself.
...
\noindent
///FIX THIS
Historically, 3D reconstruction has been performed by using photogrammetry to generate a point cloud or mesh
Traditional methods typically use explicit 3D representations such as point clouds \cite{pointCloud} or meshes \cite{mesh}.
Historically, *** SfM and MVS were used, these require large amounts of images, lots of processing and require detailed knowldge of camera locations, making it unwieldy for general and in-the-field use (for exampel ahve to setup image studio game dev)
///FIX THIS
...
Examples include F3D-Gaus TODO CITE, Gauss VideoDreamer TODO CITE, TGS TODO CITE and Splatter Image\cite{splatterImage}.
