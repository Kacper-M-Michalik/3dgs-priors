...
a pipeline, run by an orchestrator pattern notebook, 

that would set up a Virtual Machine for every model, then proceed to install the correct 
depednecies into each VM, using requiremtns files stored alongside the scripts in \verb|/geometry-priors|, and then
execute the prior generation notebooks in order to geenrate a complete, ready-to-use dataset.

CODE BLOCK - VM setup
CODE BLOCK - REQUIREMTN LOADING
CODE BLOCK - CALL MODELS
