...
\begin{figure}[H]
    ...
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/u-net.png}
...
\noindent
Our first modification is to allow the Splatter Image model to dynamically calculate and intialise 
the neccessary number of input laeyrs based on the supplied model configuration. This will allow
it to create models that use the desired combination of additional priors. 
\newline

\noindent
The second addition is of FiLM layers to support multimodal input.
CERTAIN PRIORS ARE NOT MAPS, SAY Planes or SEGEMTNAION TEXT TOKESN
THESE CAN BE SUBMITTED TO THE MODEL VIA METHDOS LIKE BROADCASTIGN WHERE YOU CAST THE same
VALUE FOR EACH PXIERL AND ADD A CHANNEl, THIS LIMITED BY NUM<BER OF KNOWN Layers
AND IS HIGHYL INEFFICIENT TOHUGH
ALTERNATIVE METHOD TO SUPPLY MULTIMODAL DATA IS TO MODULE LAYERS/CHANNELS BEWTEEN the U-NET/coNVolitonal blocks
in the model, via the use of FiLM layers.
EXPLAIN FILM
...
\noindent
An alternative to FiLM ios also consdieredm, namely cross-attention.
Also new cross-attention modules (slot in replacemnt for FiLM) inside the U-Net's blocks (in the bottleneck and decoder).
...
\label{section:lora}
To address the computational constraints of fine-tuning the full U-Net architecture, we integrated Low-Rank Adaptation (LoRA) \cite{HuLoRA} directly into our GaussianPredictor. While our implementation shares a similar class structure to the official \texttt{loralib} library \cite{hu2022lora} (utilizing mixins to wrap \texttt{Linear} and \texttt{Conv2d} layers), we manually adapted the forward pass to support the specific channel dimensions of the Splatter Image architecture (further detail is in \textbf{Section X: Manual LoRA Integration}).
...
guide reconstruction. For example teddy bears (as seen on the CO3D Teddybears dataset) have complex, convex shapes, 
...
orietneation of a surface on a per-pixel level, we considered this to be an excellent prior as it supports both complex shapes
such as teddy bears, but can equally well describe planar surface. Hence, we selected this prior as a prime candidate 
that could improve the models 3D surface reconstruction and help guide accurate hallucination in occluded regions.
For our ground truth we use \cite{xu2019disn} which provides a dataset of higher resolution images of ShapeNet models 
from \cite{ShapeNet}, each RGB image is paired with a depth map, a normal map and an albedo map at \url{https://github.com/Xharlie/ShapenetRender_more_variation}. 
...
\noindent
The quantitative degradation of \texttt{DPT\_Large} on the Hard set contradicts visual inspection. This discrepancy can be attributed to the quality of the available Ground Truth (GT) depth maps (as discussed earlier). \texttt{DPT\_Large} is what is used in the final depth prior generation, as described in \textbf{2.3.4
Selected Prior Integration}.

INSERT EXAMPLE IMAGE HERE OF DEPTH MAP PRODUCED 
...
before being fed into the network. Splatter Image uses the PyTorch DataLoader alongside custom Dataset objects 
to load batches of RGB images to perform inference on. The Dataset objects do not directly return a tensor of images, 
...
\noindent
This allowed our priors to be introduced into the evaluation code with ease, by simply creating a new custom Dataset object
\verb|srn_priors.py| that provides priors as new key/value pairs in the returned dictionary.
...
\noindent
For the training code, for the base non-LoRA training script, two changes had to be made.
The first change was to model loading, to add support for training from existing HuggingFace models from the 
base Splatter Image project.
...
\noindent
The second change was similar to that of the evaluation script, updating input preparation, the resulting updated
...
\subsubsection{FiLM experiment results}
To evaluate whether integrating different geometry priors into the model with FiLM layers (as described in TODO cite section) is effective, we performed a set of experiments testing different combinations of geometry priors. We measured the performance using PSNR, SSIM and LPIPS. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/film_psnr_distribution.png}
    \caption{PSNR distribution for different combinations of added geometry priors with FiLM}
    \label{fig:film_psnr}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/film_ssim_distribution.png}
    \caption{SSIM distribution for different combinations of added geometry priors with FiLM}
    \label{fig:film_ssim}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/film_lpips_distribution.png}
    \caption{LPIPS distribution for different combinations of added geometry priors with FiLM}
    \label{fig:film_lpips}
\end{figure}

Similar to the LoRA experiment results in \ref{section:lora_results}, using FiLM to augment the model with geometry priors makes the model perform more consistently, as shown by the lower variance in all metrics as compared to the baseline in figures \ref{fig:film_psnr}, \ref{fig:film_ssim} and \ref{fig:film_lpips}. All augmented models have a higher mean PSNR and SSIM from figures \ref{fig:film_psnr} and \ref{fig:film_ssim} respectively which suggests better image reconstruction quality, with the model adding both depth and normal priors performing the best. However, all augmented models have a higher LPIPS score than the baseline \ref{fig:film_lpips}, suggesting that the difference between the ground truth and the images generated by the model perceived by humans are larger for augmented models than the baseline. We conclude that integrating these priors with FiLM layers may not be beneficial in some cases, for example cases where successful image reconstruction is defined as being similar to the ground truth based on a human's perception.
...
Section 4.3: FILL IN
\newline
Image collection: Kacper and Radhika and Alex
\newline
Citation collection: Radhika and Alex
\newline
