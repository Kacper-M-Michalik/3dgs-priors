...
\noindent
Altogether, we managed to successfully augment the baseline Splatter Image implementation, with the geometry priors integrated in the final implementation being normal and depth maps, which were selected out of a larger subset of priors. These priors were used to augment the Splatter Image model via two methods, the first being by expanding the set of input channels and the latter being LoRA fine-tuning. We found that both of these approaches resulted in a higher reconstruction quality than the baseline as measured by PSNR.
