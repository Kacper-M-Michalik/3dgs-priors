...
\noindent
The processing scripts generate \verb|parquet| format files containing the dataset or prior information
alongside relevant metadata, such as the image frame\_id or depicted model's UUID. Namely, the 5 parquet files currently generated are:
\vspace{5pt}
...
\vspace{10pt}
\noindent
These files now provide a ready-to-use dataset. To upload the dataset to HuggingFace, additional processing must be performed
...
Specifically, we transform the dataset to be compatible with this HuggingFace YAML configuration:
...
for datatype in datatypes:
    ...
    !mkdir {datatype}
    %cd {datatype}
    load_path = os.path.join(out_folder, datatype + ".parquet")
