...
using a 2 layer FCN as the FiLM parameter generator. Splatter Image's top level \verb|GaussianSplatPredictor|
...
\noindent 
As demonstrated in Figure \ref{fig:conditional_comparison}, the model demonstrates strong performance on conditional views (the input viewpoint). In both the Geometry Priors and LoRA configurations, the reconstruction is faithful (to the human eye) to the ground truth. The model successfully encodes the input image features into the 3D Gaussian representation, capturing sharp boundaries, correct coloring, and accurate object silhouettes. 

\noindent
However, Figure \ref{fig:novel_comparison} highlights the mentioned challenges in single-view reconstruction. When rendering novel views, where angles deviated significantly from the input, we can observe a degradation in quality. While the global geometry remains consistent, high-frequency details are lost. As seen in the LoRA prediction (Figure \ref{fig:lora_novel}), the boundaries of the vehicle, such as around the wheel edges, lack the sharper definition found in the ground truth, with the edges appearing blurred. 

\noindent
Areas that were self-occluded in the input view (such as bumper edge), as can be seen in the Geometry Priors model in Figure \ref{fig:priors_novel}, are reconstructed with a more ``muddy" texture. This adds to the drop in fine detail. Therefore, artefacts common in baseline methods are not entirely resolved by the addition of priors, in this case. 
...
\noindent
We have the following distribution plots for the metrics we consider across the configurations (10k iterations):
\begin{figure}[H]
    \centering
...
\noindent
PSNR increased following finetuning, improving on Splatter Image's baseline, with the largest gain observed for the LoRA configuration with rank 32. This suggests further tests to see to what extent increased rank capacity could improve reconstruction, considering the validation loss results seen in Section \ref{sec:tt_results}. The `pairing' of the two models on the right (in terms of similar results, as in section \ref{sec:tt_results}) can also be seen here when considering the variance of these models for their PSNR.  
...
\noindent
All 3 configurations tested improved SSIM, with very similar scores, suggesting that even minimal adaptation could strengthen spatial coherence. Despite the clustering amongst results, the improvement was statistically significant, as detailed below, while no meaningful distinction was found between the parameter settings here. 
...
\noindent
With LPIPS, we see a slight decrease in all variants compared to the baseline. An LPIPS decrease would indicate improved perceptual retention, and we do see a slight decrease in all variants compared to the baseline, but as with all the configurations and metrics, further computation time would allow us to further confirm this. 
...
\noindent
With more computational resources and time, it would be beneficial to do more testing to determine the best combination of hyperparameters for the LoRA configuration.

\section{Chapter 4: Conclusions and Future Directions}
\subsection{Conclusions}

\noindent
Altogether, we managed to successfully augment the baseline Splatter Image implementation, adding dynamic channel depth initialisation and providing a basic method of for multimodal data input by via FiLM layers. We also successfully implemented training optimisation strategies such as LoRA and weight grafting. 
Geometry priors were successfully integrated into the project, with the finally selected priors being normal maps, depth maps and segmentation masks, all of which were selected out of a larger subset of priors. Two of these priors were successfully used to augment the Splatter Image model via concatenation to input RGB images, and training efficiency gains from LoRA were positive. We found that both of these approaches resulted in a higher reconstruction quality than the baseline as measured by PSNR.

\noindent
By making these lightweight improvements while preserving Splatter Image's key architecture, we increase its quality while still allowing it to maintain its original computational efficiency with its 3D Gaussian Splatting approach. 

\noindent
Particularly considering the ablation study, we found that having both depth and normal priors produced the highest quality results, as is evident in the quantitative metrics.
...
\noindent
Lack of time and compute also meant we could not afford to generate and test additional priors other than depths
and normals maps, despite having ready implementations of prior generation scripts like the \verb|generate_segmentation.py| script.
\newline
...
This made our search for priors models difficult and time-consuming, on an already very time stretched project.
\newline
...
\newline
Section 4.1: Alex and Radhika and Kacper
\newline
