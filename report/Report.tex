...
are deallocated from memory ass soon as rows are processed, this is due to the memory usage of the precomputed dataset

CODE BLOCK

\noindent
During training, model checkpoints are occasionally saved based on the training configuration. Namely, these files are saved:
...
\noindent
These are directly uploaded to HuggingFace into an appropriately named folder based on the training configuration.
Example pretrained models for the precomputed ShapeNet cars_priors dataset can be found on HuggingFace at: \url{https://huggingface.co/MVP-Group-Project/splatter-image-priors} 
...
MODELS AND TRAINING DONE USING TORCH, WANDB USED FOR LOGGING/RUN MANAGEMENT,
OmegaConf used to generate configs woth training parameters
...
WE PRODUCE READY TRAIEND MODELS TRAINDE ON TOP OF BASE WEIGHTS, PARAMS:
ITERS: 60K
PRIOR COMBOS

WERE TRAINIG NNTOEBOOK RUSN ON CLOAB WITH A100 OVERNIGHT, AVAIABLE WEIGHTS ON HF AT: X
...
All LoRA models were finetuned using the same optimizer settings as the baseline, namely Adam. Training was conducted for 10,000 iterations, as discussed later, with checkpoints saved every 1000 iterations to handle restarts. 
...
Our biggest limitations stemmed from a lack of time, compute reseources, and tooling problems.
Splatter Image, despite it's relative high oerforamcne for a deep learnign recosuntcion model, still requires high grade hardware to run,
and requires long amounts of time on said hardware to perform tasks like training. 

In terms of tooling, we used Google Colab Pro+ to give us access to A100 GPUs and 24 hour execution notebooks, despite these promises, we found 
...
(namely model training progress). This meant we could not reliably train models to achieve comparable results to the original pretrained models
which were trained continuosly on A6000 GPUs over a period of 7 days. 
...
to implement desired things like calculting our own groudn truths, or bneing able to geenrate point clouds to be able to calcualte 
Chaumffer distance as an addtional metric for all models. 

In terms of the limitations of splatter image halluciantion in hidden areas still a problem, as at end of the day you dont know what there,
lot of data cnd comptue needed to perofrm accurate reweults
ute to generate and test more models priors
