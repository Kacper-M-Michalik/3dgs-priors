...
\noindent
While these operators successfully identify major structural outlines, we found that they are at times sensitive to artifacts in the estimated depth maps. Consequently, we determined that explicit edge channels added insufficient benefits compared to other priors, and were excluded.
...
\begin{figure}[H]
    ...
    \caption{Comparison of novel view case. (a) Ground truth rendering of the target view. (b) Reconstruction by a LoRA-adapted model.}
    \label{fig:lora_qualitative_comparison}
\end{figure}

One example of evaluation output can be seen in \ref{fig:lora_qualitative_comparison}, where LoRA adapters were trained with a rank of 2, $\alpha$ of 2 (both low compared to standards) and dropout of 0.05. The LoRA-adapted model is able to reconstruct the car from a novel view while preserving details such as headlights, despite the low parameter overhead.
...
\subsubsection{Ablation study on geometry priors, U-Net integration}
\begin{figure}[H]
    \centering
...
Table \ref{tab:lora_results} presents the quantitative results for novel view synthesis after 5000 iterations for four distinct configurations.

\begin{table}[h]
    \centering
    \caption{Results for LoRA at 5,000 iterations with different hyperparameter configurations.}
...
Table \ref{tab:lora_best_vs_latest} compares the model at the ``Latest" checkpoint (10,000 iterations) against the ``Best" checkpoint selected via validation loss during training. 
\begin{table}[h]
    \centering
    \caption{Comparison of the selected LoRA configuration ($r=8, \alpha=8, 0.05$ dropout) at different  stages against the Baseline.}
    \label{tab:lora_best_latest}
    \begin{tabular}{lccc}
            \toprule
            \textbf{Model Stage} & \textbf{PSNR} $\uparrow$ & \textbf{SSIM} $\uparrow$ & \textbf{LPIPS} $\downarrow$ \\
            \midrule
            Baseline (Splatter Image) & 24.00 & 0.923 & 0.077 \\
            LoRA after 10k Iterations & 24.74 & 0.930 & 0.072 \\
            LoRA at best checkpoint & \textbf{24.81} & \textbf{0.931} & \textbf{0.068} \\
            \bottomrule
        \end{tabular}
    \end{table}

As shown in Table \ref{tab:lora_best_latest}, increasing the number of iterations does not necessarily guarantee better results. The model achieves its peak performance (PSNR 24.81) at the ``Best" checkpoint, contrasting with the 10,000 iteration mark (24.74 dB). This indicates that while LoRA adapts rapidly, prolonged training on this specific subset can lead to minor overfitting. This behaviour suggests the benefit of early stopping based on validation metrics.

The LoRA configurations tested here outperform the Splatter Image baseline, \cite{splatterImage}, which has a PSNR of 24.0 in comparison,  with LoRA results clustering tightly in the 24.7-24.8 range.


(FOLLOWING NEEDS RERUNS)
