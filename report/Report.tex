...
\noindent
Top-level Repository: \url{https://github.com/Kacper-M-Michalik/3dgs-priors}
\newline
\noindent
...
During model selection, basic notebooks would have been written to operate the relevant models. Once a model was selected,
the relevant notebook would be adapted into a high-performance script capable of processing an entire input dataset,
producing ready-processed priors for every RGB image. 
Namely, inference code would be rewritten to ensure it operates on high performance hardware such as the GPU,
and that all operations were performed in a batched manner.
...
batchwise on the device (the GPU in our case); only once processing is complete is the batch moved back to regular (host) 
...
Datasets such as SRN cars contain XXXXXXXXXXXXXXX images, each being 128 * 128 pixels, storing priors such as depths and normals
as 32-bit floats for each image would result in: XXXXXXXXXXX
\newline
...
a pipeline that runs these prior generation scripts and constructs a ready-to-use dataset. Details on the implementation of this can be found in 
\textbf{Section 2.4: Data pipelines}.
...
First congis are vierfied to be correct
dataset loaded as suual, as we provide a Dataset class as standard in proejct
datasrt laodear returen dictionary with images, poses, treansforms, we extend dict with priors key/value apirs
new priors looked up and stacked into input
