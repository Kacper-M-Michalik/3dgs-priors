...
\noindent
Splatter Image's high computational efficiency and good reconstruction quality make it a highly desirable model for one-shot 3D reconstruction.
Nonetheless, the model's occasional problems such as layout/scale drift, over-smooth geometry and poor quality hallucinations in occluded regions,
pose a serious problem in its usage and wider adoption. As stated in the introduction, we believe the cause of these reconstruction failures is twofold; first is inherent to Splatter
Image's architecture, the models choice of 3D reconstruction as a 2D-to-2D image translation task limits its ability to learn geometric priors, as the
model lacks an internal 3D representation to reason correctly about 3D structures. 
Second is that single or few RGB images simply does not have sufficient conditional
...
situations exactly like those where Splatter Image may struggle, namely when trying to accurately hallucinate occluded regions and trying to understand
complex 3D surfaces and structures. Ready availability of these models allows us to exploit them to gain additional sources of high quality geometric 
...
\noindent
As such we believe these priors will effectively help guide Splatter Image in 3D reconstruction, resulting in superior reconstruction quality
...
generating the output image M as the Splatter Image. 
For this it uses a U-Net as those have demonstrated excellent performance in image generation\cite{rombach2022highresolutionimagesynthesislatent}, and their ability to capture small image details helps to obtain higher-quality novel view reconstructions. 
...
At the final layer a convolution is used to map each feature vector to the desired number of classes.
...
\noindent
The Gaussian Splatting implementation of \cite{3dgs} is used for rasterization to generate $360^\circ$ views of the original input image from these parameters. As shown in figure \ref{fig:3dgs}, Gaussian splatting then optimizes and adaptively controls the density of this set of 3D Gaussians, using a fast tile-based renderer during optimization. The renderer allows $\alpha$-blending of anisotropic splats, respecting visibility order thanks to fast sorting. It also includes a fast backwards pass by tracking accumulated $\alpha$ values, without a limit on the number of Gaussians that can receive gradients. This helps to quickly generate new images of novel views.
\newline
...
\noindent
This modulated feature map is finally passed on within the network. The method is highly computationally efficient, 
...
Anotehr method for multi modal input is cross attention.
In cross attention we add crossss attention layers to model.
Cross attentio nworks by taking pixels in iamge, flattening them to list, 
this list is then projected using a lienar layer or matmul to matchign dimensions
with mutlimodal data shape (say list if vector embeddings), Q.
Multim odal data laso projected to K. Projected to V.
Dot product of every pixel agaisnt every q performed, nramlsied acroos
row or column? Softwmax

Cross aattention formula to calc documentclass

list of updaetd pixels reshaped back to image
More expressive than FiLM as it allows for per-pixel spatial conditioning.,
this method is consdiered state-of-the-art for high-performance conditional generation.
computaionlly intensive though, and only useful on larger multimodal data 
(so not 1 embedding), for example list of tokens geenrated from segmentaion, or larwge list of
...
for example normals are quantized from 3 * 32-bit floats to 3 * 8-bit unsigned integers; this is due to memory and storage limitations.
...
using a 2 layer FCN as the FiLM parameter generator. Splatter Image (and the SingleImageSongUNetPredictor module it uses)
only allow FiLM to be used to inject camera pose embeddings. These two modules can be modified to allow for multiple embeddings
to be passed instead. TODO
\newline
...
alongside relevant metadata. Parquet FORMAT WAS CHOSEN BECAUSE
...
specifically, their schemas are:
\vspace{5pt}
...
Specifically, we transform the dataset to be compatible with this HuggingFace dataset YAML configuration:

THIS ONE HAS NAME SPCIFIC TO ONE OF THE POSSIBLE DATASETS THAT CNA BE PROCESSED
...
More details about ShapeNet cars, which the precomputed dataset is based off, can be found in \textbf{Section 3.1: Datasets}.
\newline

\noindent
Training and evaluation scripts using PyTorch DataLoaders to load datasets into memory for inference. To allow
easy use of our precomputed dataset, we implement a custom PyTorch Dataset class, which can be plugged into 
DataLoaders without any additional code changes. Our custom Dataset class first downloads the datasets from Huggingface
and then preprocesses them to improve GETTING performance. Namely, the datasets are converted from Huggingface dataset objects
to pandas dataframes, where rows are then grouped by model UUID and the groups saved to a dictionary, allowing
efficient model to RGB image lookup. The HuggingFace datasets and redundant pandas datafrae,es 
are deallocated fro memory ass soon as rows are processed, this is due to the memory usage of the precomputed dataset

CODE BLOCK

\noindent
During training, model checkpoints are occasionalyl saved based on the training configuration. Namely these files are saved:

\begin{description}
   \item[config.yaml] Training run configuration.
   \item[model\_best.pth] Weights, PSNR and iteration count of highest scoring model from training run.
...
\noindent
These are directly uploaded to HuggingFace into an appropriately named folder based on the training congifration.
Example pretraiend models can be found for the precomptue ShapeNet cars dataset on HuggingFace at:
\url{https://huggingface.co/MVP-Group-Project/splatter-image-priors} 
...
All LoRA models were finetuned using the same optimizer settings as the baseline, using Adam. Training was conducted for up to 10,000 iterations, as discussed later, with checkpoints saved every 1000 iterations to handle restarts. 
...
\noindent
The \verb|dataloader_test.ipynb| notebook tests our custom Dataset and ready generated \verb|cars_priors| data.
...
With the successful integration of geometry priors, we can see the following example of evaluation output below:
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ExampleImageGT.png} 
        \caption{Ground Truth}
        \label{fig:gt_sample}
    \end{subfigure}
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ExampleImageP.png} 
        \caption{Trained Model Output}
        \label{fig:lora_sample}
    \end{subfigure}
    
    \caption{Comparison of novel view scenario. (a) Ground truth rendering of the target view. (b) Reconstruction by a model with modified architecture for additional prior channels.}
    \label{fig:lora_qualitative_comparison}
\end{figure}

Another example of evaluation output can be seen in Figure \ref{fig:lora_qualitative_comparison}, where LoRA adapters were trained with a rank of 2, $\alpha$ of 2 (both low compared to standards) and dropout of 0.05. The LoRA-adapted model is able to reconstruct the car from a novel view while preserving details such as headlights, despite the low parameter overhead.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/lora_example246_gt.png} 
        \caption{Ground Truth}
        \label{fig:gt_sample}
    \end{subfigure}
    \begin{subfigure}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/lora_example246_produced.png} 
        \caption{LoRA Prediction}
        \label{fig:lora_sample}
    \end{subfigure}
    
    \caption{Comparison of novel view scenario. (a) Ground truth rendering of the target view. (b) Reconstruction by a LoRA-adapted model.}
    \label{fig:lora_qualitative_comparison}
\end{figure}
...
It is important to note that the original Splatter Image implementation recommends that single-view models be trained in two stagaes, first without LPIPS followed by fine-tuning with LPIPS. However, due to the large computational overhead caused by fine-tuning with LPIPS and our lack of computational resources and time, we did not run the second stage with LPIPS. Hence, although we include LPIPS in our metrics in this section, we do not use it to evaluate our model's performance as it is not an accurate assessment.
...
To evaluate whether Low-Rank Adaptation (as described in \textbf{Section 2.3.5}) is effective, we performed a set of fine-tuning experiments on the ShapeNet-SRN Cars dataset. As mentioned in Section \ref{section:lora}, LoRA behaviour is primarily controlled by three hyperparameters: rank ($r$), alpha ($\alpha$), and dropout ($d$). We tested multiple configurations by varying these hyperparameters while keeping all other components constant, measuring performance using PSNR, SSIM, and LPIPS.
...
We can produce distribution plots for the scores files:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/LoRA_psnr_distribution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/LoRA_ssim_distribution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/stats-results/LoRA_lpips_distribution.png}
...
ShapeNet time to get access etc, reasoning for using the variation dataset rather than generating our own, given time constraints. 
...
generating and testing more priors (especially more priors oriented to hidden areas, could bring up planes again)
implementing and comparing cross-attention vs FiLM for multimodal data
