...
\noindent
When considering LoRA, the validation metrics from 5 to 10,000 iterations of fine-tuning reveal a distinct plateau, as we can note the y-axis scale as being extremely tight. Despite testing different rank configurations, we reached what appears to be the convergence limit of the weights early in the fine-tuning process, although it would be beneficial to confirm this by carrying on for longer or confirm this with other configurations. These metrics are discussed further in \textbf{Section \ref{sec:lora_results}: LoRA Experiment Results}.
