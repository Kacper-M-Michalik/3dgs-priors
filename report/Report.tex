...
\end{figure}
GaussVideoDreamer first initializes a video and inconsistency-aware Gaussian Splats from an input image.
...
\noindent
\\We also explored the utility of explicit depth discontinuity features, or depth edges. These reflect sudden changes in depth values within a depth map. These can serve as a proxy for occlusion boundaries, and we were investigated as they potentially aid in separating foreground objects from background geometry.

\noindent
We looked four classical edge detection operators applied directly to the estimated depth maps:
\begin{enumerate}
\item \textbf{Canny Edge Detector:} Detects a wide range of edges using noise reduction and gradient hysteresis CITE
\item \textbf{Sobel Operator:} Computes the gradient approximation of the image intensity function, resulting in regions highlighted that have high spatial frequency (CITE).
\item \textbf{Scharr Operator:} An optimization of the Sobel operator that provides better rotational symmetry. (CITE)
\item \textbf{Prewitt Operator:} A discrete differentiation operator similar to Sobel but using a simpler integer-valued kernel (CITE)
\end{enumerate}
...
\noindent
Although Sobel and Scharr effectively highlight gradient changes, Canny produces binary edges that often fragment under the noise inherent in monocular depth estimation.

\noindent
While these operators successfully identify major structural outlines, they are sensitive to artifacts in the estimated depth maps. Consequently, we determined that explicit edge channels added insufficient benefits compared to other priors, and were excluded.
...
\noindent
The processing scripts generate \verb|parquet| format files containing the prior information
alongside relevant metadata, such as the image frame\_id or depicted model's UUID.
...
avaiable. WE structure the dataset into the foramt matching the following YAML configuration.
