...
One issue that may be causing this problem is that the prediction of 3D Gaussians in Splatter Image is based on input image features alone, which do not have sufficient conditional information for Gaussian splatting to represent structures that are not visible in the input view \cite{shen2024pixelworth3dgaussians}. As shown in figure \ref{fig:splatter_image_problems}, for the chair input image in \ref{fig:chair_input}, Splatter Image has trouble generating the occluded chair leg as shown in \ref{fig:chair_splatter_1} and \ref{fig:chair_splatter_2}.
...
compared to when the neural network has to learn these geometric features itself.
...
representations such meshes or point clouds. The industry-standard workflow begins with Structure from Motion (SfM), 
...
While effective for static, diffuse environments, these methods struggled significantly with surfaces such as 
transparent windows or reflective metals—because they relied on strict photometric consistency and lack a mechanism to model view-dependent radiance.
ALSO REQUIRE hese require large amounts of images, lots of processing and require detailed knowldge of camera locations,
 making it unwieldy for general and in-the-field use (for exampel ahve to setup image studio game dev)

In 2020, novel view synthesis was revolutionized by the introduction of Neural Radiance Fields\cite{nerf}. In NeRF an underlying continuous volumetric scene function,
represented using a deep learning model, is optimized using a sparse set of input images with known camera poses; the input to the function/model
...
Novel views are synthesized by querying 5D coordinates along camera rays for some new camera pose, and output colours and densities are rendered into an image.
NeRF achieved state-of-the-art results and became the gold standard for novel view synthesis.

\begin{figure}[H]
    \centering
    \includegraphics[width=15cm]{images/gaussian_splatting.png}
    \caption{A diagram of 3D Gaussian Splatting from \cite{3dgs}}
    \label{fig:splatting_diagram}
\end{figure}

In 2023 the field was again revolutionized by 3D Gaussian Splatting, 
EXPLAIN
CHEAP TO Computes
EFFECTIVE
\newline
...
\noindent
Since the introduction of 3DGS a number of deep learning architectures and processing pipelines based on the method have been developed to find the most accurate and efficient implementation for high quality novel view synthesis.
Examples include F3D-Gaus\cite{f3dgaus}, Gauss VideoDreamer\cite{hao2025gaussvideodreamer3dscenegeneration}, TGS TODO CITE and Splatter Image\cite{splatterImage}.
...
SplatterImage suffers from hallucination and problems due to lack of data
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{images/splatter_image_diagram.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{images/u-net.png}
    \caption{U-net architecture taken from \cite{unet} that Song U-Net\cite{songunet} is based off. Each blue box corresponds to a multi-channel feature map, with the number of channels denoted on top of each box. The x-y size is provided at the lower left edge of the box. White boxes represent copied feature maps. The arrows denote the different operations.}
    \label{fig:u-net_diagram}
\end{figure}

ADD FIGURE OF USONGNET, DESCRIBE ARCH IN MORE DETATIL< TALK ABOUT HOW CROSS ATTENTION SUED FOR MULTI IMAGE
\newline
...
All code associated with the project can be found in the fopllowing repsoitories:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/SOD_Fbeta.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/SODPrecisionRecall.png}
...
\textbf{Manual LoRA Integration: TO BE REVISED as necessary}
To adapt the Splatter Image architecture for fine-tuning on the ShapeNet-SRN Cars dataset with depth and normal priors, we integrated Low-Rank Adaptation (LoRA). (CITE) 

Standard libraries such as Hugging Face's PEFT (CITE) provide out-of-the-box LoRA integration, we found them incompatible with the weight-grafting mechanism required by the SplatterImage pipeline. Splatter Image relies on grafting weights from pre-trained checkpoints where input channel dimensions change (e.g., when adding depth or normal map channels). This leads to shape mismatches during initialisation steps (such as for PEFT's LoRA layers) since the pre-trained weights cannot be directly loaded into layers with modified input dimensions. (CHECK THIS)

We added LoRA manually within the \texttt{GaussianPredictor} and \texttt{train\_network} modules, using code from Microsoft's \texttt{loralib} (CITE). We modified the underlying \texttt{Linear} and \texttt{Conv2d} layers of the U-Net architecture to include a \texttt{LoRALayer} mixin. This allows the freezing of pre-trained base weights while injecting trainable rank-decomposition matrices ($A$ and $B$) directly into the layer definition. 
...
Radhika - List priors, Lora integration, trainign setup, writeup
Kacper - Splatter Image Architectural changes, dataset loading, training setup, dataset generation/orchestration based on prior reserach from others, write-up
Alex - List priors, (splatter image initial bug fixes?), graphing/results code?, writeup
